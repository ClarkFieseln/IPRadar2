...
class GoogleMapPlotter(object):
    ...
    def __init__(self, center_lat, center_lng, zoom, maptype, apikey='', showHeatmap=True):
        ...
        ######################################
        # change in API from 3.10 to 3.11:
        # see "Converting MarkerImage objects to type Icon" here:
        # https://developers.google.com/maps/documentation/javascript/markers
        ######################################
        # BUG correction 2019.12.22.1441
        ######################################
        # ORG LINE: self.coloricon = os.path.join(os.path.dirname(__file__), 'markers/%s.png')
        ###self.coloricon = os.path.join(os.path.dirname(__file__), 'markers/%s.png')
        self.coloricon = '%s'
   ###     self.coloricon = os.path.join(os.path.dirname(__file__), 'markers/%s.png')
        # self.coloricon = os.path.join(os.path.dirname(__file__), 'markers/0000FF.png')
    ...
    def marker(self, lat, lng, color='#FF0000', c=None, title="no implementation", bounce=True, dot=False, tx=30, rx=30):
        # 2019.12.26
        ###############
        # if c:
        #     color = c
        ...
        self.points.append((lat, lng, color[:], title, bounce, dot, tx, rx))
        # ORG
    ...
    def write_points(self, f):
        for point in self.points:
            self.write_point(f, point[0], point[1], point[2], point[3], point[4], point[5], point[6], point[7])
    ...
    def write_map(self,  f):
        ...
        #############################################################
        # EDITED roadmap -> satellite on 2019.12.22.1512
        #############################################################
        # f.write('\t\t\tmapTypeId: google.maps.MapTypeId.ROADMAP\n')
        # TODO: hardcoded values...correct this..
        #########################################
        ...
        f.write('\t\tmap.setTilt(45);') # e.g. for mapTypeId SATELLITE and hybrid we can see 45° images
        #####################################
        # we write the function always..even if there are no bouncing markers
        # if bounce == True:
        f.write('\n')
        f.write('\t\tfunction toggleBounce() {\n')
        f.write('\t\tif (this.getAnimation() !== null) {\n')
        f.write('\t\t  this.setAnimation(null);\n')
        f.write('\t\t} else {\n')
        f.write('\t\t    this.setAnimation(google.maps.Animation.BOUNCE);\n')
        # f.write('\t\t    addMarkerWithTimeout(neighborhoods[0], 200);\n')
        f.write('\t\t    addMarkerWithTimeout(this, 1);\n')
        f.write('\t\t        }\n')
        f.write('\t\t}\n')
        f.write('\n')
        f.write('\t\tfunction addMarkerWithTimeout(marker, timeout) {\n')
        f.write('\t\twindow.setTimeout(function() {\n')
    ...
    def write_point(self, f, lat, lon, color, title, bounce, dot, tx, rx):
        # ADDED 2020.01.26
        ############
        # Horrible workaround until we find the problem !!!
        if lat == None or lon == None:
            lat = 0.1
            lon = 0.1
            print("******** ERROR: gmplot.write_point(): lat or lon None, replaced with 0.0 values!")
        ############
        f.write('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n' %
                (lat, lon))
        # BUT with Markers solved by commenting the following 3 lines
        #############################################################
        ######################################
        # change in API from 3.10 to 3.11:
        # see "Converting MarkerImage objects to type Icon" here:
        ...
        if dot == True:
            f.write('\t\tvar img = { url: \'http://maps.google.com/mapfiles/ms/icons/%s-dot.png\', scaledSize: new google.maps.Size(%d, %d) };\n' %
                (self.coloricon % color, tx, rx))
        else:
            f.write('\t\tvar img = { url: \'http://maps.google.com/mapfiles/ms/icons/%s.png\', scaledSize: new google.maps.Size(%d, %d) };\n' %
                (self.coloricon % color, tx, rx))
        # ORG:
        ...
        if bounce == True:
            # f.write('\t\tmarker.addListener(\'click\', toggleBounce);\n')
            f.write('\t\tmarker.addListener(\'mouseover\', toggleBounce);\n')
        f.write('\n')

    # NOTE: there is NO function to draw "single lines", there is only .Polyline()
    ##############################################################################
    def write_polyline(self, f, path, settings):
        ...
        f.write('icons: [{icon: lineSymbol,repeat:\'2000px\',offset:\'50%\'}],\n')
        f.write('clickable: %s,\n' % (str(clickable).lower()))
        f.write('geodesic: %s,\n' % (str(geodesic).lower()))
